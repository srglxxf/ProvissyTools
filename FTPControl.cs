...
namespace ProvissyTools
{
    ...
    public class FTPControl
    {
        ...
        public FTPControl(Uri FtpUri, string strUserName, string strPassword)
        {
        ...
        public FTPControl(Uri FtpUri, string strUserName, string strPassword, WebProxy objProxy)
        {
            this._Uri = new Uri(FtpUri.GetLeftPart(UriPartial.Authority));
            _DirectoryPath = FtpUri.AbsolutePath;
            if (!_DirectoryPath.EndsWith("/"))
        ...
        /// 新目录的名字。 
        /// 说明：如果新目录是当前目录的子目录，则直接指定子目录。如: Provissy1/Provissy2 ； 
        /// 如果新目录不是当前目录的子目录，则必须从根目录一级一级的指定。如： ./Provissy/Provissy1/FK3
        /// </param>
        ...
        #endregion
        #region Reload WebClient，FTP progress supported
