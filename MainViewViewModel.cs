...
namespace ProvissyTools
{
    public class MainViewViewModel : Grabacr07.KanColleViewer.ViewModels.TabItemViewModel
    {
        ...
        {
            get { return ProvissyToolsSettings.Current.EnableSoundNotify; }
            set
            {
                if (ProvissyToolsSettings.Current.EnableSoundNotify != value)
                {
                    ProvissyToolsSettings.Current.EnableSoundNotify = value;
        ...
        public string RemainingCount
        {
            get
            {
                ...
                var map = MapInfoProxy.Maps.api_data.LastOrDefault(x => x.api_eventmap != null);
                if (map == null) return "No Map";
                if (!MapInfo.EventBossDictionary.ContainsKey(map.api_id)) return "自己猜！";
